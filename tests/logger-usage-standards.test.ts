...
const EXCLUDED_PATTERNS = [
  ...
  /services\/sqlite\/reconciliation\/(audit-store|catalogue|cost-store|evidence-store|jobs-store|llm-caller|relations-store|scoring|settings|status-filter|status-validator|types)\.ts$/, // Leaf stores/pure rules; orchestration logs in reconciler + worker
];
