...
describe('AgentFormatter', () => {
  describe('renderAgentHeader', () => {
    it('should produce valid markdown header with project name', () => {
      ...
      expect(result[0]).toMatch(/^# \[my-project\] recent context, \d{4}-\d{2}-\d{2} \d{1,2}:\d{2}[ap]m (?:[A-Z]{3,4}|GMT[+-]\d{1,2}(?::\d{2})?)$/);
    ...
    it('should handle empty project name', () => {
      ...
      expect(result[0]).toMatch(/^# \[\] recent context, \d{4}-\d{2}-\d{2} \d{1,2}:\d{2}[ap]m (?:[A-Z]{3,4}|GMT[+-]\d{1,2}(?::\d{2})?)$/);
    });
  });
