...
export class MigrationRunner {
  ...
  runAllMigrations(): void {
    ...
    this.addObservationReconciliationSchema();
    this.addObservationStatusAuditTable();
    this.addObservationReconcileCostsTable();
  }
  ...
  private addObservationReconciliationSchema(): void {
    const applied = this.db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(35) as SchemaVersion | undefined;
    if (applied) return;

    const obsCols = this.db.query('PRAGMA table_info(observations)').all() as TableColumnInfo[];
    const existing = new Set(obsCols.map(c => c.name));

    const lifecycleColumns: Array<{ name: string; ddl: string }> = [
      { name: 'status', ddl: "ALTER TABLE observations ADD COLUMN status TEXT DEFAULT 'active'" },
      { name: 'status_confidence', ddl: 'ALTER TABLE observations ADD COLUMN status_confidence REAL DEFAULT 1.0' },
      { name: 'status_reason', ddl: 'ALTER TABLE observations ADD COLUMN status_reason TEXT' },
      { name: 'status_updated_at_epoch', ddl: 'ALTER TABLE observations ADD COLUMN status_updated_at_epoch INTEGER' },
      { name: 'superseded_by_observation_id', ddl: 'ALTER TABLE observations ADD COLUMN superseded_by_observation_id INTEGER' },
      { name: 'reconciled_at_epoch', ddl: 'ALTER TABLE observations ADD COLUMN reconciled_at_epoch INTEGER' }
    ];

    for (const col of lifecycleColumns) {
      if (!existing.has(col.name)) {
        this.db.run(col.ddl);
        logger.debug('DB', `Added ${col.name} column to observations table (migration 35)`);
      }
    }

    this.db.run('CREATE INDEX IF NOT EXISTS idx_observations_status ON observations(status)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observations_project_status ON observations(project, status, created_at_epoch DESC)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observations_superseded_by ON observations(superseded_by_observation_id)');
    ...
    `);
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_relations_source ON observation_relations(source_observation_id)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_relations_target ON observation_relations(target_observation_id)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_relations_relation ON observation_relations(relation)');
    ...
    `);
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_evidence_project ON observation_evidence(project)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_evidence_created ON observation_evidence(created_at_epoch DESC)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_evidence_pending ON observation_evidence(pending_message_id)');
    ...
    `);
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_reconcile_jobs_status ON observation_reconcile_jobs(status, created_at_epoch)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_reconcile_jobs_project_status ON observation_reconcile_jobs(project, status, created_at_epoch)');

    this.db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(35, new Date().toISOString());
    logger.debug('DB', 'Observation reconciliation schema initialized (migration 35)');
  }

  private addObservationStatusAuditTable(): void {
    ...
    `);
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_status_audit_observation ON observation_status_audit(observation_id, created_at_epoch DESC)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_status_audit_actor ON observation_status_audit(actor)');

    this.db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(36, new Date().toISOString());
    logger.debug('DB', 'Observation status audit table initialized (migration 36)');
  }

  private addObservationReconcileCostsTable(): void {
    ...
    `);
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_reconcile_costs_job ON observation_reconcile_costs(job_id)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_reconcile_costs_created ON observation_reconcile_costs(created_at_epoch DESC)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_observation_reconcile_costs_project_created ON observation_reconcile_costs(project, created_at_epoch DESC)');

    this.db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(37, new Date().toISOString());
    logger.debug('DB', 'Observation reconcile costs table initialized (migration 37)');
  }
}
