...
export function storeObservation(
  ...
  if (inserted) {
    return { id: inserted.id, createdAtEpoch: inserted.created_at_epoch, inserted: true };
  }
  ...
  return { id: existing.id, createdAtEpoch: existing.created_at_epoch, inserted: false };
}
