...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  private handleUpdateSettings = this.wrapHandler((req: Request, res: Response): void => {
    ...
    const settingKeys = [
      ...
      'CLAUDE_MEM_PI_MODEL',
      'CLAUDE_MEM_PI_THINKING',
      'CLAUDE_MEM_PI_TIMEOUT_MS',
      'CLAUDE_MEM_PI_MAX_CONTEXT_MESSAGES',
      'CLAUDE_MEM_PI_MAX_TOKENS',
  ...
  private validateSettings(settings: any): { valid: boolean; error?: string } {
    ...
    const validProviders = ['claude', 'gemini', 'openrouter', 'pi'];
    if (!validProviders.includes(settings.CLAUDE_MEM_PROVIDER)) {
      return { valid: false, error: 'CLAUDE_MEM_PROVIDER must be "claude", "gemini", "openrouter", or "pi"' };
      }
    }
    ...
    if (settings.CLAUDE_MEM_FALLBACK_CHAIN) {
      const validProviderIds = ['antigravity-tm-sonnet', 'antigravity-ghul-sonnet', 'antigravity-tm', 'antigravity-ghul', 'codex-spark', 'codex-mini'];
