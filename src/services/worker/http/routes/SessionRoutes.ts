...
import { PiProvider, isPiSelected, isPiAvailable } from '../../PiProvider.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getSelectedProvider(): 'claude' | 'gemini' | 'openrouter' | 'pi' {
  ...
  private async startGeneratorWithProvider(
    ...
    const agent = provider === 'pi' ? this.piAgent : (provider === 'openrouter' ? this.openRouterAgent : (provider === 'gemini' ? this.geminiAgent : this.sdkAgent));
    const agentName = provider === 'pi' ? 'Pi' : (provider === 'openrouter' ? 'OpenRouter' : (provider === 'gemini' ? 'Gemini' : 'Claude SDK'));
