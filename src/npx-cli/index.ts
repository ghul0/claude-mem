...
${styleText('bold', 'Install Commands')} (no Bun required):
  ...
  ${styleText('cyan', 'npx claude-mem install --provider claude|gemini|openrouter|pi')}   Set LLM provider non-interactively
...
${styleText('bold', 'IDE Identifiers')}:
  claude-code, cursor, opencode, pi, openclaw,
...
function parseInstallOptions(argv: string[]): InstallOptions {
  ...
  if (provider !== undefined && provider !== 'claude' && provider !== 'gemini' && provider !== 'openrouter' && provider !== 'pi') {
    console.error(`Unknown --provider: ${provider}. Allowed: claude, gemini, openrouter, pi`);
    process.exit(1);
  }
